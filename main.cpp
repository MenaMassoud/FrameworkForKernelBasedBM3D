...
int main(int argc, char **argv)
{
   //! Check if there is the right call for the algorithm
	...
	//! Declarations
	vector<float> img, img_noisy, img_denoised,img_basic, img_bias, img_diff;
   ...
   char buffer[31]="";
 //! Load image
  char* path = "IMAGES/StandardTestImages/";
  char* extension = ".png";
for(int itr = 7; itr <= 7; itr++){
sprintf (buffer, "%s%d%s", path, itr, extension);
 cout<<buffer<<endl;//instrad of buffer img name
	if(load_image(buffer, img, &width, &height, &chnls) != EXIT_SUCCESS)
       return EXIT_FAILURE;
	//! Variables initialization
	//float          fSigma       = atof(argv[2]);
	const bool     useSD_1      = (bool) atof(argv[3]);
	const bool     useSD_2      = (bool) atof(argv[5]);
	...
	const unsigned tau_2D_wien  = (strcmp(argv[4], "dct" ) == 0 ? DCT :
                                  (strcmp(argv[4], "bior") == 0 ? BIOR : NONE));
    if (tau_2D_wien == NONE)
    {
	...
	const unsigned color_space  = (strcmp(argv[6], "rgb"  ) == 0 ? RGB   :
                                  (strcmp(argv[6], "yuv"  ) == 0 ? YUV   :
                                  (strcmp(argv[6], "ycbcr") == 0 ? YCBCR :
                                  (strcmp(argv[6], "opp"  ) == 0 ? OPP   : NONE))));
    if (color_space == NONE)
    {
	...
	bool           compute_bias = (bool) atof(argv[1]);
	...
	if (compute_bias)
	{
	 ...
	 for(float fSigma = 20 ;  fSigma <= 80 ; fSigma+=20){
	//! Add noise
    ...
    //! Denoising
    if (run_bm3d(fSigma, img, img_noisy, img_basic, img_denoised, width, height, chnls,
                 useSD_1, useSD_2, tau_2D_hard, tau_2D_wien, color_space,itr)
        != EXIT_SUCCESS)
        return EXIT_FAILURE;
    ...
    // cout << "PSNR denoised: " << psnr << endl;
    // cout << "RMSE denoised: " << rmse << endl << endl;
    ...
    if (compute_bias)
    {
        if (run_bm3d(fSigma, img, img, img_basic_bias, img_bias, width, height, chnls,
                     useSD_1, useSD_2, tau_2D_hard, tau_2D_wien, color_space,itr)
            != EXIT_SUCCESS)
            return EXIT_FAILURE;
