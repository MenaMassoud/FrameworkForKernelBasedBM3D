...
/**
 * @brief Estimate the threshold using otsu method
 *
 * @param sigma: estimated standard deviation of the noise;
 * @param sigma_Y : noise on the first channel;
 * @param sigma_U : (if chnls > 1) noise on the second channel;
...
{
    //compute histogram
int MAXINTENSITY = *max_element(begin(input_img),end(input_img));
unsigned hist[MAXINTENSITY+1];
int value;
 // Initialize array
for(int i = 0; i <= MAXINTENSITY; i++) hist[i] = 0;

for(int i = 0; i< width; i++){
    for(int j = 0; j < height ; j++){
 value = (int) input_img[i + j *width];
hist[value]++;
}
}
 // Compute threshold
...
for(int i = 0; i <= MAXINTENSITY ; i++){
    ...
    //update the val between class variance
  ...
  //cout<<"fun Threshold  = "<<threshold<<endl;
  return threshold;
